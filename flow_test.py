...
class RealNVP():
    # @C.Function
    @classmethod
    def basic_network(cls, dims: int):
        return C.layers.Sequential([
                C.layers.Dense(8, C.tanh),
                C.layers.Dense(8, C.tanh),
                C.layers.Dense(dims, None),
            ])
    ...
    def forward_network(cls, input_dim: int):# , batch_norm: bool = False):
        ...
        chunk['log_s_func'] = _log_s_func = cls.basic_network(_half_dim)
        chunk['t_func'] = _t_func = cls.basic_network(_half_dim)
        ...
        chunk['log_s_func2'] = _log_s_func2 = cls.basic_network(_half_dim)
        chunk['t_func2'] = _t_func2 = cls.basic_network(_half_dim)
